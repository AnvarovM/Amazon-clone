    ...
    <header>
      ...
      <div className="flex items-center bg-amazon_blue p-1 flex-row py-2">
        ...
        <div className="hidden sm:flex items-center h-10 rounded-md flex-grow cursor-pointer bg-yellow-400 hover:bg-yellow-500">
          <input
            className="p-2 h-full w-6 flex-grow focus:outline-none flex-shrink px-4 rounded-l-md"
            type="text"
            placeholder="Search"
          />
        ...
        <div className="text-white flex items-center text-xs space-x-6 mx-4 md:mx-6 whitespace-nowrap">
          ...
          <div onClick={() => router.push('/checkout')} className="link relative flex items-center">
            <span className="absolute top-0 right-0 md:right-10 h-4 w-4 bg-yellow-500 text-center rounded-full font-bold">
              {items.length}
            </span>
